...
- `render.yaml` … 静的クライアント(monster-3d) ＋ 中継サーバー(monster3d-server-agc) の2サービス構成
...
Name = **`monster3d-server-agc`**（クライアントの `DEFAULT_WS` と一致させる）/ Build `npm install` / Start `npm start`。
